...
OP_RESULT BTreeLL::insert(u8* o_key, u16 o_key_length, u8* o_value, u16 o_value_length)
{
   ...
   jumpmuTry()
   {
      ...
      if (config.enable_wal) {
         ...
         auto populate_wal_insert_entry = [&](WALInsert& wal_entry) {
         ...
         if (ppl_success) {
            ...
            populate_wal_insert_entry(*wal_entry);
         }
         wal_entry.submit();
      } else {
                                         ...
                                         UpdateSameSizeInPlaceDescriptor& update_descriptor)
{
   ...
   jumpmuTry()
   {
      ...
      if (config.enable_wal) {
         ...
         if (config.discardable && bf->isDiscardable() && FLAGS_per_page_logging) {
            bool entry_overrides_previous = false;
            if (FLAGS_opportunistic_log_compaction && bf->lastLogRecord() != nullptr) {
               entry_overrides_previous = (std::memcmp(bf->lastLogRecord(), wal_entry.entry, prefix_length) == 0);
            }
            auto ppl_wal_entry = bf->reservePPLEntry<WALUpdate>(payload_size, entry_overrides_previous);
            if (ppl_wal_entry) {
               if (entry_overrides_previous) {
                  u8* dst = ppl_wal_entry.value()->payload + prefix_length;
                  u8* src = wal_entry->payload + prefix_length;
                  // byte by byte bitwise XOR.
                  for (u32 i = 0; i < update_descriptor.diffLength(); ++i) {
                     dst[i] ^= src[i];
                  }
                  WorkerCounters::myCounters().consecutive_same_key_in_page++;
               } else {
                  std::memcpy(ppl_wal_entry.value(), wal_entry.entry, wal_entry_size);
               }
            } else {
               ensure(!bf->isDiscardable());
            }
         }
         // after copying the log record to the PPL, it is safe to unlock the hole.
         wal_entry.submit();
      } else {
         callback(current_value.data(), current_value.length());
         iterator.markAsDirty();
...
OP_RESULT BTreeLL::remove(u8* o_key, u16 o_key_length)
{
   ...
   jumpmuTry()
   {
      ...
      if (config.enable_wal) {
         const u16 payload_size = o_key_length + value.length();
         const u16 wal_entry_size = sizeof(WALRemove) + payload_size;
         auto populate_wal_remove_entry = [&] (WALRemove& wal_entry) {
         ...
         if (ppl_success) {
            ...
            populate_wal_remove_entry(*wal_entry);
         }
         wal_entry.submit();
      } else {
         iterator.markAsDirty();
...
// -------------------------------------------------------------------------------------
u64 BTreeLL::countInner()
{
   return BTreeGeneric::countInner();
}
// -------------------------------------------------------------------------------------
...
// -------------------------------------------------------------------------------------
void BTreeLL::redo(void *btree_node_ptr, const u8* log_record_ptr, const u8 nb_log_records, const u16 total_log_records_size)
{
   u32 offset = 0;
   u8 i = 0;
   ...
   while (i++ < nb_log_records) {
      ensure_lt(offset, total_log_records_size);
      const WALEntry *wal_entry = reinterpret_cast<const WALEntry*>(log_record_ptr + offset);
      if (static_cast<u8>(wal_entry->type) == 0) __asm__ volatile("int3");
      ensure_equal(wal_entry->magic_debugging_number, WAL_BTREE_MAGIC);
   ...
   ensure_equal(offset, total_log_records_size);
