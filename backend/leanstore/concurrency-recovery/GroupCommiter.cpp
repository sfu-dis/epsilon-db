...
void CRManager::groupCommiter()
{
   ...
   ready_to_commit_rfa_cut.resize(log_manager->log_count, 0);
   ...
   while (keep_running) {
      ...
      for (u32 log_i = 0; log_i < log_manager->log_count; log_i++) {
         Logging& logging = log_manager->all_logs[log_i];
         {
            ...
            ready_to_commit_rfa_cut[log_i] = logging.precommitted_queue_rfa.size();
         }
         ...
         min_all_workers_hardened_commit_ts = std::min<TXID>(min_all_workers_hardened_commit_ts, log2gct.precommitted_tx_commit_ts);
      ...
      for (u32 log_i = 0; log_i < log_manager->log_count; log_i++) {
         Logging& logging = log_manager->all_logs[log_i];
         logging.hardened_commit_ts.store(wt_to_lw_copy[log_i].precommitted_tx_commit_ts, std::memory_order_release);
         ...
         // TODO: prevent contention on mutex
         {
            ...
            u64 tx_i = 0;
            for (tx_i = 0;
                 tx_i < logging.precommitted_queue.size() && logging.precommitted_queue[tx_i].max_observed_gsn <= min_all_workers_gsn &&
                 logging.precommitted_queue[tx_i].start_ts <= min_all_workers_hardened_commit_ts;
                 tx_i++) {
               logging.precommitted_queue[tx_i].state = Transaction::STATE::COMMITTED;
            }
            if (tx_i > 0) {
               signaled_up_to = std::min<TXID>(signaled_up_to, logging.precommitted_queue[tx_i - 1].commitTS());
               logging.precommitted_queue.erase(logging.precommitted_queue.begin(), logging.precommitted_queue.begin() + tx_i);
               committed_tx += tx_i;
            }
            // -------------------------------------------------------------------------------------
            for (tx_i = 0; tx_i < ready_to_commit_rfa_cut[log_i]; tx_i++) {
               logging.precommitted_queue_rfa[tx_i].state = Transaction::STATE::COMMITTED;
            }
            if (tx_i > 0) {
               signaled_up_to = std::min<TXID>(signaled_up_to, logging.precommitted_queue_rfa[tx_i - 1].commitTS());
               logging.precommitted_queue_rfa.erase(logging.precommitted_queue_rfa.begin(),
                                                           logging.precommitted_queue_rfa.begin() + tx_i);
               committed_tx += tx_i;
            }
         }
         if (signaled_up_to < std::numeric_limits<TXID>::max() && signaled_up_to > 0) {
            logging.signaled_commit_ts.store(signaled_up_to, std::memory_order_release);
         }
      }
